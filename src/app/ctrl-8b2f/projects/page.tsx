...
export default function ProjectsPage() {
  ...
  const fetchProjects = async () => {
    const res = await fetch('/api/ctrl-8b2f/projects')
  ...
  const handleDelete = async () => {
    if (!deleteId) return
    await fetch(`/api/ctrl-8b2f/projects/${deleteId}`, { method: 'DELETE' })
    setDeleteId(null)
    fetchProjects()
  }
              ...
              <tr key={project.id} className="border-b border-fv-border/50 hover:bg-fv-dark/50 transition">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Link
                      href={`/ctrl-8b2f/projects/${project.id}/edit`}
