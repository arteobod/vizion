...
export default function ProcessPage() {
  ...
  const updateStep = (index: number, field: string, value: string | string[]) => {
    const updated = [...steps]
    if (typeof value === 'string' && (field === 'deliverables' || field === 'deliverables_ru' || field === 'deliverables_lv')) {
      updated[index] = { ...updated[index], [field]: (value as string).split(',').map(d => d.trim()) }
    } else {
  ...
  const handleSave = async () => {
    setSaving(true)
    await fetch('/api/ctrl-8b2f/process', {
      method: 'PUT',
        ...
        {steps.map((step, index) => (
          <div key={index} className="bg-fv-surface border border-fv-border rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className={lang === 'EN' ? '' : 'md:col-span-3'}>
                ...
                <input
                  type="text"
                  value={lang === 'RU' ? (step.title_ru ?? '') : lang === 'LV' ? (step.title_lv ?? '') : step.title}
                  onChange={(e) => updateStep(index, lang === 'RU' ? 'title_ru' : lang === 'LV' ? 'title_lv' : 'title', e.target.value)}
                  placeholder={lang !== 'EN' ? step.title : undefined}
            ...
            <div className="mt-4">
              ...
              <textarea
                rows={2}
                value={lang === 'RU' ? (step.description_ru ?? '') : lang === 'LV' ? (step.description_lv ?? '') : step.description}
                onChange={(e) => updateStep(index, lang === 'RU' ? 'description_ru' : lang === 'LV' ? 'description_lv' : 'description', e.target.value)}
                placeholder={lang !== 'EN' ? step.description : undefined}
            ...
            <div className="mt-4">
              <label className="block font-mono text-xs text-fv-text-dim mb-1 tracking-wider">
                DELIVERABLES (comma-separated) {lang !== 'EN' && <span className="text-fv-orange">[{lang}]</span>}
              </label>
              <input
                type="text"
                value={
                  lang === 'RU' ? (step.deliverables_ru ?? []).join(', ')
                  : lang === 'LV' ? (step.deliverables_lv ?? []).join(', ')
                  : step.deliverables.join(', ')
                }
                onChange={(e) => updateStep(index, lang === 'RU' ? 'deliverables_ru' : lang === 'LV' ? 'deliverables_lv' : 'deliverables', e.target.value)}
                placeholder={lang !== 'EN' ? step.deliverables.join(', ') : undefined}
                className="w-full px-3 py-2 bg-fv-dark border border-fv-border rounded-lg font-mono text-sm text-fv-text outline-none focus:ring-1 focus:ring-fv-orange focus:border-fv-orange transition placeholder:text-fv-text-muted"
              />
            </div>
          </div>
