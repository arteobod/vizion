...
function ServiceCard({ service, index }: { service: Service; index: number }) {
  ...
  const title = (locale === 'ru' && service.title_ru) ? service.title_ru : (locale === 'lv' && service.title_lv) ? service.title_lv : service.title
  const description = (locale === 'ru' && service.description_ru) ? service.description_ru : (locale === 'lv' && service.description_lv) ? service.description_lv : service.description
  const metricLabel = service.metricLabel
  const { ref, isVisible } = useScrollReveal<HTMLDivElement>({ threshold: 0.05, rootMargin: '0px 0px 80px 0px' })

  return (
    <TiltCard maxTilt={8}>
      <div
        ref={ref}
        className={`grid-cell p-8 lg:p-12 group cursor-default h-full${index % 2 === 0 ? ' md:border-r-0' : ''}`}
        style={{
      ...
      >
        ...
        <h3 className="font-mono text-2xl lg:text-3xl font-bold text-fv-white leading-tight whitespace-pre-line mb-6 group-hover:text-fv-orange transition-colors duration-500">
          <TextScramble text={title} delay={index * 200} scrambleDuration={600} />
        </h3>
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
        ...
        >
          <div className="flex items-center gap-6">
            <span className="font-mono text-micro text-fv-text-muted">{t.services.sectionLabel}</span>
            <span
            ...
            />
            <TextScramble text={t.services.title} className="font-mono text-label text-fv-text-dim" delay={200} />
          </div>
